...
The search functionality issue has been fixed. Please update to the latest version from the [releases page](https://github.com/josprox/Estrella-Music/releases/latest).
...
for issue_num in "${SEARCH_ISSUES[@]}"; do
    ...
    if gh issue comment "${issue_num}" --body "${COMMENT}" --repo josprox/Estrella-Music; then
        echo "  ✓ Comment added to issue #${issue_num}"
        
        # Close the issue
        if gh issue close "${issue_num}" --reason completed --repo josprox/Estrella-Music; then
